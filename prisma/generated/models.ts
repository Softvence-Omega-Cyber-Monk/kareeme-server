
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/PaymentMethod'
export type * from './models/PrivacySettings'
export type * from './models/Statement'
export type * from './models/Transaction'
export type * from './models/DealStatus'
export type * from './models/StatementRelease'
export type * from './models/StatementTerritory'
export type * from './models/StatementPlatform'
export type * from './models/PaymentRequest'
export type * from './models/TeamMember'
export type * from './models/DashboardStats'
export type * from './models/RecentActivity'
export type * from './models/PlatformAnalytics'
export type * from './models/Asset'
export type * from './models/GeoTrend'
export type * from './models/Claim'
export type * from './models/UserOtp'
export type * from './models/RefreshToken'
export type * from './models/Distribution'
export type * from './models/PlatformDistribution'
export type * from './models/DistributionNote'
export type * from './models/DistributorClient'
export type * from './models/ClientBackCatalogue'
export type * from './models/FileInstance'
export type * from './models/VideoMergeJob'
export type * from './models/LoginDevice'
export type * from './models/Notification'
export type * from './models/UserNotification'
export type * from './models/NotificationSettings'
export type * from './models/Release'
export type * from './models/Artist'
export type * from './models/ReleaseArtist'
export type * from './models/Label'
export type * from './models/ReleaseTerritory'
export type * from './models/SplitSheetAgreement'
export type * from './models/Contributor'
export type * from './models/Track'
export type * from './models/TrackArtist'
export type * from './models/Publisher'
export type * from './models/BackCatalogue'
export type * from './models/GlobalSettings'
export type * from './models/ClientSettings'
export type * from './models/User'
export type * from './commonInputTypes'